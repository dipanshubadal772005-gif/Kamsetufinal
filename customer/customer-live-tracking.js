/* =====================================================
   KAMSETU CUSTOMER LIVE TRACKING - DEMO
...
function initializeCustomerTracking() {
    setBookingDetails();
    initializeMap();
    ...
    console.log("KAMSETU demo tracking initialized.");
...
function setBookingDetails() {
    ...
    if (providerName) {
        providerName.textContent = DEMO_BOOKING.provider.name;
    }
    ...
    if (arrivalMessage) {
        arrivalMessage.textContent =
            "Provider is on the way from Sector 52, Gurgaon";
    }
...
function initializeMap() {
    const mapElement = document.getElementById("providerMap");

    if (!mapElement) {
        console.error("Customer tracking map element not found.");
        return;
    }
...
function updateDistanceAndETA(distanceKm, etaMinutes) {
    ...
    const etaElement =
        document.getElementById("providerETA");
